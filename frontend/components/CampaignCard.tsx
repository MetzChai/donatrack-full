...
const CampaignCard: React.FC<CampaignCardProps> = ({ campaign }) => {
  ...
  return (
    <Link href={`/campaigns/${campaign.id}`}>
      <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition-shadow cursor-pointer h-full flex flex-col border border-gray-200">
        {campaign.imageUrl && (
          <img
          ...
          <div className="mt-auto">
            <div className="flex justify-between items-center mb-2">
              ...
              <span className="text-sm font-semibold text-gray-800">
                ${campaign.collected.toFixed(2)}
              </span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2 mb-2">
              <div
                className={`h-2 rounded-full ${
                  percentage >= 65 ? "bg-green-600" : "bg-red-500"
                }`}
                style={{ width: `${progress}%` }}
              />
            </div>
            <div className="flex justify-between items-center">
              <span className="text-sm text-gray-600">Goal: ${campaign.goalAmount.toFixed(2)}</span>
              <span
            ...
            {campaign.isEnded && (
              <p className="text-red-500 text-xs mt-1">• Campaign Ended</p>
            )}
          </div>
        </div>
      </div>
